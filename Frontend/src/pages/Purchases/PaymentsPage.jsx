...
import { 
    FiPlus, FiSearch, FiCreditCard, FiTrendingUp,
                                            ...
                                            <td className="py-4 px-6">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center text-white font-semibold text-sm">
                                                        {(payment.supplier?.displayName || payment.supplier?.companyName || payment.supplier?.firstName || 'V').charAt(0)}
                                                    </div>
                                                    <div>
                                                        <div className="font-medium text-gray-900">{payment.supplier?.displayName || payment.supplier?.companyName || `${payment.supplier?.firstName || ''} ${payment.supplier?.lastName || ''}`.trim() || 'Unknown'}</div>
                                ...
                                <div key={payment.id} className="p-4 hover:bg-gray-50 transition-colors">
                                    <div className="flex items-start justify-between mb-3">
                                        <div>
                                            ...
                                            <div className="text-sm text-gray-500 mt-1">{payment.supplier?.displayName || payment.supplier?.companyName || `${payment.supplier?.firstName || ''} ${payment.supplier?.lastName || ''}`.trim() || 'Unknown'}</div>
                                        </div>
